...
import {GoSwapShopRelayerGovernance, IWormhole, IERC20, SafeERC20} from "./GoSwapShopRelayerGovernance.sol";
import {GoSwapShopRelayerMessages, BytesLib} from "./GoSwapShopRelayerMessages.sol";

/**
 * @title A Cross-Chain GoSwapShopRelayer Application
 * @notice This contract uses Wormhole's token bridge contract to send tokens
 * cross chain with an aribtrary message payload.
 */
contract GoSwapShopRelayer is 
    UUPSUpgradeable,
    GoSwapShopRelayerGovernance,
    GoSwapShopRelayerMessages,
    ReentrancyGuardUpgradeable 
{
     ...
     * feePrecision and relayerFeePercentage state variables. See GoSwapShopRelayerState.sol
    ...
    function initialize(
        ...
        require(signatureVerifier_ != address(0), "invalid signature verifier address");
        ...
        nonExistingOrder(orderId_) 
    {
        ...
        SafeERC20.safeTransferFrom(
            ...
            msg.sender,
            address(this),
            calculateProduct(_newOrder.amount, _newOrder.fee, _newOrder.orderOwner)
        );
        ...
        external
    {
        ...
        uint256 productAmount = calculateProduct(_order.amount, _order.fee, _order.orderOwner);
        _order.amount = 0;
        ...
        external
    {
        ...
        SafeERC20.safeTransfer(IERC20(token_), msg.sender, productAmount);
        // take the fee
        SafeERC20.safeTransfer(IERC20(token_), owner(), totalBalance - productAmount);
     ...
     * @notice Transfers specified tokens to any registered GoSwapShopRelayer contract
        ...
        returns (uint64 messageSequence)
    {
        ...
        uint256 amountReceived = calculateProduct(_orderData.amount, _orderData.fee, _orderData.orderOwner);
        _orderData.amount = 0;

        /**
         * Encode instructions (GoSwapShopRelayerMessage) to send with the token transfer.
         ...
         * than 20 bytes.
         */
        bytes memory messagePayload = encodePayload(GoSwapShopRelayerMessage({
            payloadID: 1,
            targetRecipient: targetRecipient_
        }));
     ...
     * `GoSwapShopRelayerMessage` payload with additional transfer instructions.
     */
    function redeemTransferWithPayload(bytes memory encodedTransferMessage) public {
        ...
        // confirm that the message sender is a registered GoSwapShopRelayer contract
        if (transfer.fromAddress != getRegisteredEmitter(parsedMessage.emitterChainId)) {
            revert Forbidden("emitter not registered");
        }

        // parse the GoSwapShopRelayerMessage payload from the `TransferWithPayload` struct
        GoSwapShopRelayerMessage memory goSwapShopRelayerMsg = decodePayload(
            transfer.payload
        );
        ...
        address recipient = bytes32ToAddress(goSwapShopRelayerMsg.targetRecipient);
    ...
    function calculateFee(uint256 product, uint8 fee) public pure returns (uint256) {
        // Range from 0.01% -> 100%
        (bool isProductSafe, uint256 productWifFee) = Math.tryMul(product, uint256(fee));
        if (isProductSafe == false) {
            revert Forbidden("product wif fee is not safe");
        }

        (bool isFeeSafe, uint256 feeAmount) = Math.tryDiv(productWifFee, 10_000);
        if (isFeeSafe == false) {
    ...
    function calculateFeeRate(uint256 product, MakerOrTaker direction) public pure returns (uint8 fee) {
    ...
    function _isValidSignature(
        ...
        bytes calldata signature_
         ...
         bytes32 encodeData = keccak256(abi.encodePacked(
            orderId_,
            amount_,
            token_,
            msg.sender
