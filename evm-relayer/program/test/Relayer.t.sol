...
// import "../src/relayer/RelayerStructs.sol";
// import "../src/relayer/Relayer.sol";
...
contract RelayerTest {
    ...
    //     uint256 totalAmount = relayer.calculateProduct(
    //         amount,
    //         takerFee,
    //         RelayerStructs.MakerOrTaker.Taker
    //     );
    //     // wrap some avax
    //     wrapAvax(totalAmount);
    //     // approve the Relayer to spend wavax
    ...
    //     vm.startPrank(theBuyer);
    //     relayer.confirmTheOrder(orderId);
    ...
    //     relayer.createAnOrder(orderId, amount);
    //     vm.stopPrank();
    // }
    ...
    //     relayer.claimTheOrder(orderId, token, claimTheOrderSignature);
    ...
    // //  * deserialize the RealyerVault message correctly.
    // //  */
    ...
    // //     bytes memory encodedMessage = relayerVault.encodePayload(
    // //         orderId,
    // //         targetRecipient
    // //     );
    // //     // decode the message by calling the decodePayload method
    // //     (bytes32 _orderId, bytes32 _targetRecipient) =
    // //         relayerVault.decodePayload(encodedMessage);
    ...
    //         token
    //     );
    //     // fund to relayer. Just test for now
    //     vm.startPrank(address(relayer));
    //     vm.deal(address(relayer), 100 ether);
    //     // wrap some avax
    //     wrapAvax(amount);
    //     console.logString("Funded to Relayer");
    //     vm.stopPrank();
    ...
    //     relayer.registerEmitter(
    //         targetChainId,
    //         ethereumEmitter
    //     );
    //     vm.stopPrank();
    //     vm.startPrank(theBuyer);
    //     uint64 sequence = relayer.sendTokensWithPayload(
    ...
    //      */
    //     uint256 _transferAmount = relayer.calculateProduct(
    //         amount,
    //         takerFee,
    //         RelayerStructs.MakerOrTaker.Taker
    //     );
    ...
    //     assertEq(transfer.tokenChain, relayer.chainId(), "invalid chain");
    //     assertEq(transfer.to, ethereumEmitter, "");
    //     assertEq(transfer.toChain, targetChainId, "invalid chain");
    //     assertEq(transfer.fromAddress, addressToBytes32(address(relayer)), "invalid from");
    ...
    //     assertEq(wormholeMessage.consistencyLevel, relayer.wormholeFinality(), "invalid consistency level");
    //     // parse additional payload and verify the values
    //     RelayerStructs.RelayerMessage memory _msg =
    //         relayer.decodePayload(transfer.payload);
    ...
    // // }
    // /**
    //  * @notice This test confirms that Relayer correctly redeems wrapped
    ...
    //     vm.assume(
    //         amount > 1e10 &&
    //         amount < type(uint256).max / relayer.relayerFeePercentage()
    //     );
    ...
    //     // );
    //     // encode the message by calling the encodePayload method
    //     bytes memory encodedRelayerMessage = relayer.encodePayload(
    //         RelayerStructs.RelayerMessage({
    //             payloadID: uint8(1),
    //             targetRecipient: addressToBytes32(address(relayer))
    //         })
    //     );
    ...
    //             to: addressToBytes32(address(relayer)),
    //             toChain: relayer.chainId(),
    //             fromAddress: ethereumEmitter,
    //             payload: encodedRelayerMessage
    //         });
    ...
    //     // register the emitter on the source contract
    //     vm.startPrank(defaultOwnerAddress);
    //     relayer.registerEmitter(ethereumChainId, ethereumEmitter);
    //     vm.stopPrank();
    //     // store balances in the Balances struct (reduces local variable count)
    //     Balances memory balances;
    //     balances.recipientBefore = getBalance(wrappedAsset, address(relayer));
    ...
    //     relayer.redeemTransferWithPayload(signedMessage);
    //     // check balance of wrapped weth after redeeming the transfer
    //     balances.recipientAfter = getBalance(wrappedAsset, address(relayer));
    //     // balances.relayerAfter = getBalance(wrappedAsset, vm.addr(guardianSigner));
    //     // // compute the relayer fee
    //     // uint256 relayerFee = relayer.calculateRelayerFee(denormalizedAmount);
