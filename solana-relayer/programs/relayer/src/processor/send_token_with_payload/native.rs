...
pub struct SendNativeTokensWithPayload<'info> {
    ...
    #[account(
        address = config.token_bridge.custody_signer @ RelayerError::InvalidTokenBridgeCustodySigner
    )]
...
pub fn send_native_tokens_with_payload(
    ...
    require!(truncated_amount > 0, RelayerError::ZeroBridgeAmount);
    require!(
        valid_foreign_address(ctx.accounts.order_account.chain_id, &recipient_address),
        RelayerError::InvalidRecipient
    );
    require!(
        recipient_chain == ctx.accounts.order_account.chain_id,
