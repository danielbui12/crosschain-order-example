...
) -> Result<u64, GoSwapShopError> {
    let fee_amount = calculate_fee(product, fee)?;
    let result: u64 = match direction {
        MakerOrTaker::Taker => product.checked_add(fee_amount).ok_or(GoSwapShopError::ProductOverflow)?,
        MakerOrTaker::Maker => product.checked_sub(fee_amount).ok_or(GoSwapShopError::ProductOverflow)?,
    };
    
    Ok(result)
}

pub fn calculate_fee(product: u64, fee: u8) -> Result<u64, GoSwapShopError> {
    // Range from 0.01% -> 100%
    Ok(product.checked_mul(fee as u64)
           .and_then(|result| result.checked_div(10_000))
           .ok_or(GoSwapShopError::FeeOverflow)?)
}
