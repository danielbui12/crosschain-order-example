...
use anchor_spl::{
    token::{Mint, Token, TokenAccount, Transfer, CloseAccount, close_account, transfer},
};
...
pub fn claim_the_order(
    ctx: Context<ClaimTheOrder>,
    _order_salt: Vec<u8>,
) -> Result<()> {
    ...
    let total_balance = token_bridge::truncate_amount(calculate_product(
            truncated_amount,
            calculate_fee_rate(truncated_amount, MakerOrTaker::Taker),
            MakerOrTaker::Taker
        )?, ctx.accounts.mint.decimals);
    let product = calculate_product(truncated_amount, order_account.fee, order_account.order_owner.clone())?;
