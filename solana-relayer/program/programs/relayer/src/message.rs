...
impl AnchorDeserialize for RelayerMessage {
    fn deserialize(buf: &mut &[u8]) -> io::Result<Self> {
        match buf[0] {
            PAYLOAD_ID_HELLO => Ok(RelayerMessage::Msg {
...
pub type PostedRelayerMessage = token_bridge::PostedTransferWith<RelayerMessage>;
...
pub mod test {
    ...
    fn test_message_alive() -> Result<()> {
        ...
        let msg = RelayerMessage::Msg { recipient };
        ...
        let RelayerMessage::Msg {
            recipient: decoded_recipient,
        } = RelayerMessage::deserialize(&mut encoded.as_slice())?;
