...
pub struct RedeemNativeTransferWithPayload<'info> {
    ...
    // #[account(
    //     mut,
    //     constraint = seller.key() == recipient.key() || payer_token_account.key() == anchor_spl::associated_token::get_associated_token_address(&seller.key(), &mint.key()) @ RelayerError::InvalidPayerAta
    // )]
