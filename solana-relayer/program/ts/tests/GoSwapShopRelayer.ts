...
import * as goSwapShopRelayer from "../sdk/swapshop_relayer";
...
describe("GoSwapShopRelayer", function () {
  ...
  console.log(
    "goSwapShop operator pubkey:",
    goSwapShopOperator.publicKey.toString(),
  );
  ...
  const program = goSwapShopRelayer.createGoSwapShopRelayerProgramInterface(
    connection,
  ...
  describe("Initialize Program", function () {
    ...
    const createInitializeIx = (opts?: {
      ...
      goSwapShopRelayer.createInitializeInstruction(
        connection,
        GOSWAPSHOP_RELAYER_ID,
        owner.publicKey,
        TOKEN_BRIDGE_PID,
    ...
    it("Finally Initialize Program", async function () {
      ...
      const senderConfigData = await goSwapShopRelayer.getSenderConfigData(
        connection,
        GOSWAPSHOP_RELAYER_ID,
      );
      ...
      const redeemerConfigData = await goSwapShopRelayer.getRedeemerConfigData(
        connection,
        GOSWAPSHOP_RELAYER_ID,
      );
  ...
  describe("Update Relayer Fee", function () {
    ...
    const createUpdateRelayerFeeIx = (opts?: {
      ...
      goSwapShopRelayer.createUpdateRelayerFeeInstruction(
        connection,
        GOSWAPSHOP_RELAYER_ID,
  ...
  describe("Register Foreign Emitter", function () {
    const createRegisterForeignContractIx = (opts?: {
      ...
      goSwapShopRelayer.createRegisterForeignContractInstruction(
        connection,
        GOSWAPSHOP_RELAYER_ID,
        opts?.sender ?? owner.publicKey,
        TOKEN_BRIDGE_PID,
        ...
        await expectIxToFailWithError(
          await program.methods
            ...
            .accounts({
              ...
              config: goSwapShopRelayer.deriveSenderConfigKey(
                GOSWAPSHOP_RELAYER_ID,
              ),
              foreignContract: goSwapShopRelayer.deriveForeignContractKey(
                GOSWAPSHOP_RELAYER_ID,
                chain,
              ),
    ...
    [Buffer.alloc(32, "fbadc0de", "hex"), foreignContractAddress].forEach(
      (contractAddress) =>
        it(`Register ${contractAddress === foreignContractAddress ? "Final" : "Random"} Address`, async function () {
          ...
          const { chain, address } =
            await goSwapShopRelayer.getForeignContractData(
              connection,
              GOSWAPSHOP_RELAYER_ID,
              foreignChain,
            );
    ...
    const payload = parseTokenTransferPayload(
      (
        await wormhole.getPostedMessage(
          connection,
          goSwapShopRelayer.deriveTokenTransferMessageKey(
            GOSWAPSHOP_RELAYER_ID,
            sequence,
          ),
        )
  ...
  (
    ...
    const DEFAULT_ORDER_DATA: goSwapShopRelayer.OrderParams = {
      makerFee: 100n,
      takerFee: 100n,
      ...
      amount: sendAmount,
      claimer: goSwapShopRelayer.Claimer.Buyer,
    ...
    const createCreateAnOrderIx = (
      ...
      goSwapShopRelayer.createCreateAnOrderInstruction(
        connection,
        GOSWAPSHOP_RELAYER_ID,
    ...
    const createPlaceAnOrderIx = (
      ...
      goSwapShopRelayer.createPlaceAnOrderInstruction(
        connection,
        GOSWAPSHOP_RELAYER_ID,
    ...
    const createCancelTheOrderIx = (
      ...
      goSwapShopRelayer.createCancelTheOrderInstruction(
        connection,
        GOSWAPSHOP_RELAYER_ID,
        opts?.sender ?? buyer.publicKey,
        mint,
    ...
    const createConfirmTheOrderIx = (
      ...
      goSwapShopRelayer.createConfirmTheOrderInstruction(
        connection,
        GOSWAPSHOP_RELAYER_ID,
        opts?.sender ?? buyer.publicKey,
        seller.publicKey,
    ...
    const createClaimTheOrderIx = (
      ...
      goSwapShopRelayer.createClaimTheOrderInstruction(
        connection,
        GOSWAPSHOP_RELAYER_ID,
        seller.publicKey,
        mint,
    ...
    const logBalance = async (key: string, orderSalt: Buffer) => {
      ...
      try {
        const tmpAccount = goSwapShopRelayer.deriveTmpTokenAccountKey(
          GOSWAPSHOP_RELAYER_ID,
          mint,
          buyer.publicKey,
          orderSalt,
    ...
    //     transaction.partialSign(goSwapShopOperator);
    ...
    describe(`For ${isNative ? "Native" : "Wrapped"} With ${decimals} Decimals`, function () {
      describe(`Send Tokens With Payload`, function () {
        ...
        const createSendTokensWithPayloadIx = (opts?: {
          ...
          (isNative
            ? goSwapShopRelayer.createSendNativeTokensWithPayloadInstruction
            : goSwapShopRelayer.createSendWrappedTokensWithPayloadInstruction)(
            connection,
            GOSWAPSHOP_RELAYER_ID,
            opts?.sender ?? buyer.publicKey,
            TOKEN_BRIDGE_PID,
        ...
        it("Finally Send Tokens With Payload", async function () {
          ...
          const buyerTmpAccount = goSwapShopRelayer.deriveTmpTokenAccountKey(
            GOSWAPSHOP_RELAYER_ID,
            goSwapShopOperator.publicKey,
            buyer.publicKey,
            ORDER_SALT,
          ...
          //   goSwapShopRelayer.deriveTmpTokenAccountKey(
          //     GOSWAPSHOP_RELAYER_ID,
          //     goSwapShopOperator.publicKey,
          //     buyer.publicKey,
          //     ORDER_SALT
      ...
      describe(`Redeem Tokens With Payload`, function () {
        ...
        const receiveAmount = calculateProduct(
          ...
          goSwapShopRelayer.MakerOrTaker.Taker,
        );
          
        const createRedeemTransferWithPayloadIx = (signedMsg: Buffer) =>
          (isNative
            ? goSwapShopRelayer.createRedeemNativeTransferWithPayloadInstruction
            : goSwapShopRelayer.createRedeemWrappedTransferWithPayloadInstruction)(
            connection,
            GOSWAPSHOP_RELAYER_ID,
            seller.publicKey,
            goSwapShopOperator.publicKey,
        ...
        const publishAndSign = (opts?: {
          ...
          const published = foreignTokenBridge.publishTransferTokensWithPayload(
            ...
            GOSWAPSHOP_RELAYER_ID.toBuffer().toString("hex"),
        ...
        //       ? goSwapShopRelayer.getCompleteTransferNativeWithPayloadCpiAccounts
        //       : goSwapShopRelayer.getCompleteTransferWrappedWithPayloadCpiAccounts)(
        //         TOKEN_BRIDGE_PID,
        //         CORE_BRIDGE_PID,
        //         seller.publicKey,
        //         parsed,
        ...
        //       );
        //     const orderAccount = goSwapShopRelayer.deriveOrderKey(
        //       GOSWAPSHOP_RELAYER_ID,
        //       goSwapShopOperator.publicKey,
        //       seller.publicKey,
        //       ORDER_SALT
        ...
        //         config: goSwapShopRelayer.deriveRedeemerConfigKey(GOSWAPSHOP_RELAYER_ID),
        //         foreignContract:
        //           goSwapShopRelayer.deriveForeignContractKey(GOSWAPSHOP_RELAYER_ID, parsed.emitterChain as ChainId),
